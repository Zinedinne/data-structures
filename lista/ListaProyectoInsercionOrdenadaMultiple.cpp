...
void Lista::insertarNombre(){ //si se elige priorizar nombre se usa este metodo
    ...
    if(estaVacia()){ //si la lista esta vacia
        ...
        if(cab->getSig()==NULL || cab->getNombre()>p->getNombre()){//si tiene solo un elemento
                if(cab->getNombre()>p->getNombre()){//el nuevo es menor
                p->setSig(cab);
                cab=p;
            }else{
                if(cab->getNombre()<p->getNombre()){//el nuevo es mayor
                    cab->setSig(p);
                    }else{//el nuevo es igual
                        if(cab->getMatricula()>p->getMatricula()){//se empieza a validar por matricula, si la nueva matricula es menor
                            p->setSig(cab);
                            cab=p;
                        }else{
                            cab->setSig(p);   //el otro caso es que la matricula sea mayor, ya no se profundiza el siguiente criterio porque las matriculas no deberian repetirse
                        }
                    }     
                }
        }else{//La lista tiene mas de un elemento
                while(aux1->getSig()!=NULL && aux1->getSig()->getNombre()<p->getNombre()){
                    aux1=aux1->getSig();
                    }
                if(aux1->getSig()!=NULL && aux1->getSig()->getNombre()!=p->getNombre()){//si el semestre no es mayor a todos los de la lista y el siguente es diferente
                    p->setSig(aux1->getSig());
                    aux1->setSig(p);
                    }
                if(aux1->getSig()!=NULL && aux1->getSig()->getNombre()==p->getNombre()){//si el semestre no es mayor a todos los de la lista y el siguiente es igual
                    aux2=aux1;
                    while(aux2->getSig()!=NULL && aux2->getSig()->getMatricula()<p->getMatricula()){
                        aux2=aux2->getSig();
                        }
                    p->setSig(aux2->getSig());
                    aux2->setSig(p);
                    
                    }
                if(aux1->getSig()==NULL){
                    aux1->setSig(p);//si es mayor a todos los anteriores
                    }
        }
    }
}
...
void Lista::insertarSemestre(){ //si se elige priorizar semestre se usa este metodo
    ...
    if(estaVacia()){ //si la lista esta vacia
        ...
        if(cab->getSig()==NULL || cab->getSemestre()>p->getSemestre()){//si tiene solo un elemento
            if(cab->getSemestre()>p->getSemestre()){//el nuevo es menor
                p->setSig(cab);
                cab=p;
            }else{
                if(cab->getSemestre()<p->getSemestre()){//el nuevo es mayor
                    cab->setSig(p);
                    }else{//el nuevo es igual
                        if(cab->getMatricula()>p->getMatricula()){//se empieza a validar por matricula, si la nueva matricula es menor
                            p->setSig(cab);
            ...
            while(aux1->getSig()!=NULL && aux1->getSig()->getSemestre()<p->getSemestre()){
                    aux1=aux1->getSig();
                    }
                if(aux1->getSig()!=NULL && aux1->getSig()->getSemestre()!=p->getSemestre()){//si el semestre no es mayor a todos los de la lista y el siguente es diferente
                    p->setSig(aux1->getSig());
                    aux1->setSig(p);
                    }
                if(aux1->getSig()!=NULL && aux1->getSig()->getSemestre()==p->getSemestre()){//si el semestre no es mayor a todos los de la lista y el siguiente es igual
                    aux2=aux1;
                    while(aux2->getSig()!=NULL && aux2->getSig()->getMatricula()<p->getMatricula()){
                        aux2=aux2->getSig();
                        }
